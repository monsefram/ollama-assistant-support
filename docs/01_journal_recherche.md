...
| 2026-06-03 | Réparation de la lecture vocale (qui ne lisait plus les réponses) et distinction reconnaissance / synthèse | Après le passage hors-ligne, la lecture à voix haute des réponses ne marchait plus. J'ai d'abord pensé remplacer « la voix Google » par faster-whisper, mais en cherchant j'ai compris une distinction importante : faster-whisper sert à la RECONNAISSANCE vocale (transformer ma voix en texte, pour le micro), pas à la SYNTHÈSE (lire un texte à voix haute). Les deux sont opposées. Le vrai problème venait du fait que ma voix sélectionnée était « Google français », une voix RÉSEAU qui échoue sans connexion. J'ai modifié le code pour qu'il privilégie toujours une voix LOCALE (Microsoft), avec une bascule automatique si la voix choisie échoue. J'ai aussi étiqueté les voix « locale » ou « en ligne » dans le menu. | C'est une leçon utile : il y a deux mondes vocaux séparés, la reconnaissance (STT) et la synthèse (TTS). Pour un projet 100 % local, le point faible restant est en fait le MICRO (la reconnaissance), qui sur Chrome envoie ma voix à Google. faster-whisper serait pertinent là, mais demanderait un serveur Python à côté d'Ollama — une piste pour plus tard. Pour la lecture, le simple fait de forcer une voix locale règle le problème et garde le projet hors-ligne. À documenter dans le rapport comme limite et piste d'amélioration. |
| 2026-06-03 | Calibration du Face ID pour le rendre robuste à l'éclairage | J'avais remarqué que mon Face ID marchait bien chez moi mais plus du tout dans un autre endroit avec une lumière différente. En cherchant pourquoi, j'ai compris que la librairie (face-api.js) transforme le visage en une « empreinte » de 128 nombres calculée à partir des contrastes de l'image : quand la lumière change, les ombres bougent, l'empreinte change, et la distance avec mon empreinte de référence dépasse le seuil de reconnaissance. Mon code ne gardait qu'UNE seule empreinte (une seule lumière). Sur le conseil de mon prof, j'ai mis en place une calibration : on enregistre maintenant PLUSIEURS empreintes dans des conditions différentes (lumière de face, de côté, plus faible, angle), et à la vérification je compare avec toutes et je garde la distance la plus petite. | C'est le même principe qu'un vrai Face ID d'iPhone, qui demande de tourner la tête en cercle pour capturer plein de variations. Tant qu'une seule empreinte de référence est proche, le visage est reconnu, donc ça marche dans plusieurs endroits. Ça illustre bien la démarche du projet : j'ai observé une limite réelle (sensibilité à la lumière), j'ai compris sa cause technique (empreinte basée sur les contrastes), puis je l'ai corrigée (calibration multi-échantillons). À noter : ça reste un prototype éducatif, une webcam classique restera toujours moins fiable qu'une caméra infrarouge. |
